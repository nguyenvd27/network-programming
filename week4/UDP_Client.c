...
int main(int argc, char *argv[]){
  ...
  bytes_sent = sendto(client_sock, "Connected\n", strlen("Connected\n"), 0, (struct sockaddr *) &server_addr, sin_size);
  bytes_received = recvfrom(client_sock, buff, BUFF_SIZE - 1, 0, (struct sockaddr *) &server_addr, &sin_size);
  buff[bytes_received] = '\0';
  int check = 0; // dung de phan biet client1, client2
  if(strstr(buff, "nguyenvd27_!@#$&_client1") != NULL){
    check = 1;
    printf("Sender\n");
  }else if(strstr(buff, "nguyenvd27_!@#$&_client2") != NULL){
    check =2;
    printf("Receiver\n");
  }
  else if(strstr(buff, "nguyenvd27_!@#$&_client3") != NULL){
    check = 3;
    printf("Error. Client > 2\n");
    return 0;
  }

  while(1){
    if(check == 1){ // client1 la sender
      printf("Send:\n");
    ...
    }else if(check == 2){ // client2 la receiver
      bytes_received = recvfrom(client_sock, buff, BUFF_SIZE - 1, 0, (struct sockaddr *) &server_addr, &sin_size);
      if(bytes_received < 0){
        perror("Error: ");
        close(client_sock);
