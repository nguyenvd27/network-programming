...
int main(int argc, char *argv[]){
  ...
  int client_count = 0;
  int check_client1 = 0, check_client2 = 0; // check su ton tai cua client1, client2
  ...
  bzero(&(server.sin_zero),8); // thuoc tinh ko su dung, reset ve zero
  ...
  while(1){
    ...
    if(client_count == 0){// check so luong client ket noi
      client1 = client;
      check_client1 = 1;
      strcpy(buff, "nguyenvd27_!@#$&_client1");
      bytes_sent = sendto(server_sock, buff, strlen( buff), 0, (struct sockaddr *) &client, sin_size);
    }else if(client_count == 1 && ntohs(client.sin_port) != ntohs(client1.sin_port)){
      client2 = client;
      check_client2 = 1;
      strcpy(buff, "nguyenvd27_!@#$&_client2");
      bytes_sent = sendto(server_sock, buff, strlen( buff), 0, (struct sockaddr *) &client, sin_size);
    }else if(client_count == 2 && ntohs(client.sin_port) == ntohs(client1.sin_port)){
      int check_error = parse_string( buff);
      if(check_error == 0){
        strcpy(buff, "Error");
        bytes_sent = sendto(server_sock, buff, 5, 0, (struct sockaddr *) &client2, sin_size); // gui error cho client
      }
      else{
        bytes_sent = sendto(server_sock, buff, bytes_received, 0, (struct sockaddr *)&client2, sin_size); // gui du lieu da xu ly cho clien
      }
    }
    ...
    if(ntohs(client.sin_port) != ntohs(client1.sin_port) && ntohs(client.sin_port) != ntohs(client2.sin_port)){
      strcpy(buff, "nguyenvd27_!@#$&_client3");
      bytes_sent = sendto(server_sock, buff, strlen( buff), 0, (struct sockaddr *) &client, sin_size);
    }
    if(bytes_sent < 0)
      perror("\nError: ");          
